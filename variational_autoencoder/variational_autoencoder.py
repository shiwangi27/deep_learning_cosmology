...
def get_image_array(X, index, shp=(128,128), channels=1):
    ret = (X[index]).reshape(channels,shp[0],shp[1])
    if channels == 1:
...
def build_vae(inputvar, L=2, binary=True, imgshape=(128,128), channels=1, z_dim=2, n_hid=4096):
    ...
    l_enc_hid = nn.layers.DenseLayer(l_input, num_units=n_hid,      ### Dense layer with hid_units = 4096 
    ...
    for i in range(L):
        ...
        if binary:
            ...
            print("DECODER MU --------------------------------------", l_dec_mu_list.shape)
...
def main(L=2, z_dim=2, n_hid=4096, num_epochs=300, binary=True, sample_size=1000,  outfolder='VarAETester'):
    print("Loading data...")
    X_train, X_val, X_test = load_data(sample_size)
    ...
    for epoch in range(num_epochs):
        ...
        val_err = 0
        val_batches = 0
        for batch in iterate_minibatches(X_val, val_batch_size, shuffle=False):
            err = val_fn(batch)
            val_err += err
            val_batches += 1
    ...
    test_err = 0
    test_batches = 0
    for batch in iterate_minibatches(X_test, val_batch_size, shuffle=False):
        err = val_fn(batch)
        test_err += err
        test_batches += 1
    ...
    outURL = '/global/homes/s/ssingh79/variational_autoencoder/output_files/' + outfolder
    if not os.path.isdir(outURL):
        os.mkdir(outURL)
    pred_fpath = outURL + '/pred_images.h5'  
    gen_fpath = outURL + '/gen_images.h5'
    
    
    # save some example pictures so we can see what it's done 
    example_batch_size = 5
    X_comp = X_train[:example_batch_size]
    pred_fn = theano.function([input_var], test_prediction)
    X_pred = pred_fn(X_comp).reshape(-1, 1, width, height)
    #for i in range(5):
    #    get_image_pair(X_comp, X_pred, idx=i, channels=1).save(outURL + '/output_'+str(i)+'.png')
    ...
    print("Saving")
    fn = outURL + '/params_{:.6f}'.format(test_err)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_folder', type=str, dest='outfolder')
    parser.add_argument('--L', type=int, dest='L')
    parser.add_argument('--z_dim', type=int, dest='z_dim')
    parser.add_argument('--n_hid', type=int, dest='n_hid') ## Try 1024 and 2048, 4096 
    parser.add_argument('--binary', dest='binary', action='store_true')
    parser.add_argument('--continuous', dest='binary', action='store_false')
    parser.set_defaults(binary=False) ## Try both binary and continuous!
