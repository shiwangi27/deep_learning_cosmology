...
def load_data(sample_size=100, split_percent = 0.8, outfolder='Conv_ae_output'):
    ...
    def reshape_data(train_set):
        ...
        visualize_normalized_data(valid_x.reshape(-1,1,128,128), outfolder)
...
def build_conv_ae(input_var=None):
    ...
    num_units_dense_1 = 128*128
    num_units_dense_2 = 64*64
    
    # Input layer, specifying the expected input shape of the network
    # (unspecified batchsize, 1 channel, 128 rows and 128 columns) and
    # linking it to the given Theano variable `input_var`, if any:
    
    l_in = lasagne.layers.InputLayer(shape=(None,128*128),
                                     input_var=input_var)
    
    print("Input shape : ", l_in.output_shape )
    
    l_in_corrup = lasagne.layers.DropoutLayer(l_in, p=0.3)
    ...
    l_dense_1 = lasagne.layers.DenseLayer(l_in_corrup, 
                                          num_units = num_units_dense_1,  
                                        nonlinearity=lasagne.nonlinearities.rectify
                                        )
    
    print("Dense layer 1 : ", l_dense_1.output_shape)
    
    
    # Add the First Deconv layer -- (64, 128, 128)
    l_dense_2 = lasagne.layers.DenseLayer(l_dense_1,
                                          num_units = num_units_dense_2,
                                    nonlinearity = lasagne.nonlinearities.rectify
                                        )
    
    print("Dense layer 2 : ", l_dense_2.output_shape)
    
    # Add the First Deconv layer -- (64, 128, 128)
    l_dense_3 = lasagne.layers.DenseLayer(l_dense_2,
                                          num_units = num_units_dense_1,
                                    nonlinearity = lasagne.nonlinearities.rectify
                                        )
    
    print("Dense layer 2 : ", l_dense_3.output_shape)
    
    print("Final layer Shape : ", l_dense_3.output_shape)
    print("Final layer weights : ", l_dense_3.W.get_value())
...
def main(num_epochs = 500, learning_rate=0.1, batch_size = 80, sample_size = 100, split_percent=0.8, outfolder = 'Conv_ae_output'):
    # Load the dataset
    print("Loading data...")
...
if __name__ == '__main__': 
    ...
    if len(sys.argv) > 1:
        ...
        main(sample_size=100, batch_size = 80, outfolder = 'Conv_ae_output_fully_conn')  
