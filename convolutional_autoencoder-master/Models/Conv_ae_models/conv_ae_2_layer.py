...
def build_conv_ae(input_var=None):
    ...
    # Add First Convolution2D layer -- (64, 64, 36)
    ...
    # Add Second Convolution2D Layer -- (32, 32, 16)
    l_conv_second = lasagne.layers.Conv2DLayer(l_conv_first, 
                                               num_filters=dense_num_filter, 
                                               filter_size=filter_size,
                                               nonlinearity = lasagne.nonlinearities.rectify,
                                               stride=(2,2),
                                               pad=2)
    ...
    l_dense_1 = lasagne.layers.DenseLayer(l_conv_second, 
                                       num_units=hidden_units, 
                                       nonlinearity=lasagne.nonlinearities.rectify, 
                                       W=lasagne.init.GlorotUniform()) 
    
    print("Dense layer 1 : ", l_dense_1.output_shape)
    print("Get bottleneck layer vector --- ", lasagne.layers.get_output(l_dense_1))
    ...
    # Add the Second Deconv layer -- (64, 64, 36)
...
def main(num_epochs = 300, learning_rate=0.05, batch_size = 80, sample_size = 100, split_percent=0.8, outfolder = 'Conv_ae_output'):
    # Load the dataset
    print("Loading data...")
...
if __name__ == '__main__': 
    ...
    if len(sys.argv) > 1:
        ...
        main(sample_size=100, batch_size = 80, outfolder = 'Conv_ae_output_2_layer')  
