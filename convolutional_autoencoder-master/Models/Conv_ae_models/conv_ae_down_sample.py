...
def build_conv_ae(input_var=None, use_dropout=False):
    ...
    conv_num_filter_1 = 40
    conv_num_filter_2 = 60
    conv_num_filter_3 = 100 # Change from 4 to 8 
    conv_num_filter_4 = 100
    dense_num_filter = 8
    filter_size = 5
    hidden_units = 2048 
    ...
    # Add Second Convolution2D Layer -- (32, 32, 32)
    ...
    # Add Third Convolution2D Layer -- (16, 16, 16) 
    l_conv_third = lasagne.layers.Conv2DLayer(l_conv_second, 
                                               num_filters=conv_num_filter_3, 
                                               filter_size=filter_size,
                                               nonlinearity = lasagne.nonlinearities.rectify,
                                               stride=(2,2),
                                               pad=2)
    ...
    # Add Second Convolution2D Layer -- (8, 8, 4)
    l_conv_fourth = lasagne.layers.Conv2DLayer(l_conv_third, 
                                               num_filters=conv_num_filter_4, 
                                               filter_size=filter_size,
                                               nonlinearity = lasagne.nonlinearities.rectify,
                                               stride=(2,2),
                                               pad=2)
    ...
    # Dense layer shape -- ( Vector of 4096 ) 
    
    #l_dense_1 = lasagne.layers.DenseLayer(l_conv_third, 
    ...
    #l_reshape = lasagne.layers.ReshapeLayer(l_dense_1,
    #                                               shape=(([0], dense_num_filter, 16, 16)))
   
    #print("Reshape after Dense Layer : ", l_reshape.output_shape)
    
    # Damn! This is so hard-coded!!!
    
    # Add the First Deconv layer -- (16, 16, 8)
    l_deconv_first = lasagne.layers.Deconv2DLayer(l_conv_fourth,
                                                num_filters= conv_num_filter_3,
    ...
    # Add the Second Deconv layer -- (32, 32, 16)
    ...
    print("Deconv layer 3 : ", l_deconv_fourth.output_shape)
    ...
    return l_deconv_fourth, l_conv_fourth, l_conv_first   
...
def main(num_epochs = 300, learning_rate=0.5, batch_size = 80, sample_size = 100, split_percent=0.8, L2_lam = 0.001, use_dropout=False, outfolder = 'Conv_ae_output'):
    # Load the dataset
    print("Loading data...")
    ...
    updates = lasagne.updates.nesterov_momentum(
            loss, params, learning_rate=learning_rate, momentum=0.6) # [0.5, 0.9, 0.95, 0.99] 
    ...
    layer_1_activation = lasagne.layers.get_output(layer_active_1, deterministic=True)
    active_fn = theano.function([input_var], layer_1_activation)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_train, batch_size, shuffle=False):
            inputs = batch
        ...
        if(np.mod(epoch,10)==0):
            ...
            active_images = active_fn(X_train)
            visualize_activations(active_images, outfolder)
    ...
    visualize_tsne(bn_vector.reshape(-1,100*8*8), outfolder)
...
if __name__ == '__main__': 
    ...
    if len(sys.argv) > 1:
        ...
        lr = float(sys.argv[4])
        output = sys.argv[5]
        main(num_epochs=epoch, sample_size=samples, batch_size = batches, learning_rate=lr, outfolder = output)      
    else:
        main(sample_size=1000, batch_size = 128, use_dropout=False, L2_lam = 0.0001, outfolder = 'Conv_ae_output_downsample_test')  
