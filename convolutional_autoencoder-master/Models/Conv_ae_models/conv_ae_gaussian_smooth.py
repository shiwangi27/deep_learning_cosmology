...
def load_data(sample_size=100, split_percent = 0.9, outfolder='Conv_ae_output'):
    ...
    hdf5file = 'gaussian_smooth.h5'
    filepath = os.path.join(dataurl, hdf5file)
    
    outpath = outputURL + outfolder
    ...
    def reshape_data(train_set, sample_size):
        ...
        # Perform Gaussian smoothing on the Reshaped Images. 
        ## smooth with Gaussian filter of radius 2pix for both axes
        print("RESHAPE SIZEEEEEEEEEE ", train_x.shape)
    ...
    train_x, valid_x = reshape_data(train_set, sample_size)
...
def main(num_epochs = 500, learning_rate=0.1, batch_size = 90, sample_size = 100, split_percent=0.9, outfolder = 'Conv_ae_output'):
    ...
    reconsr_err_fname = outputURL + outfolder + 'reconst_error.npz'
    np.savez(reconsr_err_fname, cost_plot) 
    
    # Optionally, you could now dump the network weights to a file like this:
    model_fname = outputURL + outfolder + 'model.npz'
    np.savez(model_fname, *lasagne.layers.get_all_param_values(network))
...
if __name__ == '__main__': 
    ...
    if len(sys.argv) > 1:
        ...
        main(sample_size=100, batch_size = 90, outfolder = 'Conv_ae_output_gaussian')  
